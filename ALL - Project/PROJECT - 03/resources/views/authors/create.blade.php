...
@section('content')
    <div class="flex items-center space-x-3 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Create Author</h2>
            <p class="text-gray-600">Add a new author to the system</p>
        </div>
    </div>
                ...
                <form method="POST" action="{{ route('authors.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="bio" rows="5"
                            ...
                            placeholder="Write a brief biography of the author...">{{ old('bio') }}</textarea>
                        @error('bio')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
