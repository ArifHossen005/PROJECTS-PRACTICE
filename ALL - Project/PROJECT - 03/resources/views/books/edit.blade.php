        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('books.update', $book->id) }}"
                    ...
                    <div>
                        ...
                        <div class="flex items-start space-x-6">
                            <div class="flex-shrink-0">
                                <div id="imagePreview"
                                     class="w-28 h-40 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden bg-gray-100">
                                    @if($book->cover_image)
                                        <img id="previewImg" src="{{ asset('storage/' . $book->cover_image) }}"
                                             alt="Current cover" class="w-full h-full object-cover"/>
                                    @else
                                        <svg id="previewIcon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 text-gray-300">
                            ...
                            <div class="flex-1">
                                <div class="border-2 border-dashed border-gray-300 rounded-xl p-5 text-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition-all"
                                     onclick="document.getElementById('cover_image').click()">
                                    <input type="file" id="cover_image" name="cover_image"
                                           accept="image/jpeg,image/png,image/jpg"
                                           class="hidden" onchange="previewImage(event)"/>
                                    <p class="text-sm text-gray-600">
                                        <span class="font-medium text-indigo-600">Click to change</span> cover image
                                    </p>
                                    <p class="text-xs text-gray-500 mt-1">JPEG, PNG, JPG up to 2MB. Leave empty to keep current image.</p>
                                </div>
                                @error('cover_image')
                    ...
                    {{-- Author & Category - Active State Dropdowns --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
